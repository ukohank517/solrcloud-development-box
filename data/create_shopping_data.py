...
df_examples = pd.read_parquet(
    f"{INPUT_FOLDER}/shopping_queries_dataset_examples.parquet"
)
df_products = pd.read_parquet(
    f"{INPUT_FOLDER}/shopping_queries_dataset_products.parquet"
)
...
for i in range(0, len(df_examples_products), SPLIT_SIZE):
    ...
    with open(
        f"{OUTPUT_FOLDER}/shopping_products_{str(i // SPLIT_SIZE).zfill(3)}.json", "w"
    ) as outputFile:
        json.dump(
            json.loads(
                df_examples_products[data_from:data_to].to_json(orient="records")
            ),
            outputFile,
            indent=2,
        )
    print(
        f"\rcreating json file {i//SPLIT_SIZE} / {len(df_examples_products)//SPLIT_SIZE}",
        end="",
    )
